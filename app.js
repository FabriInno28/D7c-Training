...
function renderHome(){
 $('#home').innerHTML=`<header class="top"><div class="brand"><b>10'</b> MINUTEN</div><div class="micro">FÜR DICH · FÜR DEIN SPIEL</div></header><section class="hero"><div class="micro">DEIN TRAINING HEUTE</div><h1>STARK.<br>SCHNELL.<br>KONZENTRIERT.</h1><p>Wähle dein Training. Vier Übungen: Kraft, Schnellkraft, Balance, Aufmerksamkeit. Klare Signale, Timer läuft mit.</p></section><section class="world">${Object.entries(DATA).map(([k,s])=>`<button class="world-card" data-k="${k}"><div class="world-image"><img src="./${s.hero}" alt="${s.label}"></div><div class="world-body"><span class="tag">${s.label}</span><h2>${s.claim}</h2><p>${s.sub}</p><div class="world-meta"><span>${s.rounds} RUNDEN</span><span>CA. ${timeLabel(total(s))}</span></div></div></button>`).join('')}</section>`;
