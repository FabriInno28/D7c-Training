...
function renderHome(){
 const hero={stark:'stark-kniebeuge.jpg',schnell:'schnell-knie-hoch.jpg',ruhig:'ruhig-einbeinstand.jpg'};
 $('#home').innerHTML=`<header class="top"><div class="brand"><b>10'</b> MINUTEN</div><div class="micro">FÜR DICH · FÜR DEIN SPIEL</div></header><section class="hero"><div class="micro">DEIN TRAINING HEUTE</div><h1>STÄRKER.<br>SCHNELLER.<br>RUHIGER.</h1><p>Wähle, was du heute brauchst. Vier Übungen. Kurze Zeiten. Klare Signale.</p></section><section class="world">${Object.entries(DATA).map(([k,s])=>`<button class="world-card" data-k="${k}"><div class="world-image"><img src="./${hero[k]}" alt="${s.label}"></div><div class="world-body"><span class="tag">${s.label}</span><h2>${s.claim}</h2><p>${s.sub}</p><div class="world-meta"><span>${s.rounds} RUNDEN</span><span>CA. ${timeLabel(total(s))}</span></div></div></button>`).join('')}</section>`;
...
function openTraining(k){
 ...
 $('#training').innerHTML=`<header class="top"><button class="back" id="back">← AUSWAHL</button><div class="micro">${s.label}</div></header><div class="training-head"><div><span class="tag">${s.label}</span><h1>${s.claim}</h1><p>${s.sub}</p><div class="pills"><span class="pill">${s.rounds} Runden</span><span class="pill">${s.exercise_pause} Sek. Pause</span><span class="pill">${s.round_pause} Sek. Rundenpause</span><span class="pill">ca. ${timeLabel(total(s))}</span></div></div><button class="start-all" id="startAll">GANZES TRAINING STARTEN</button></div><section class="exercise-grid">${s.exercises.map((e,i)=>`<button class="exercise-card" data-i="${i}"><div class="pic"><img src="./${e.image}" alt="${e.name}"></div><div class="copy"><div class="row"><span class="num">${i+1}</span><span class="time">${cardTime(e)}</span></div><h3>${e.name}</h3><div class="cues"><span><b>1.</b> ${e.cue}</span><span><b>2.</b> ${e.focus}</span></div></div></button>`).join('')}</section><section class="finish"><div><strong>ABSCHLUSS</strong><h3>Ruhig atmen · ${s.breath} Sek.</h3><p>Durch die Nase ein. Länger aus. Puls runter. Kopf ruhig.</p></div><div>◎</div></section><section class="timer" id="timerBox"><div class="timer-top"><div><div class="micro">AKTUELL</div><div class="phase">Bereit.</div></div><div class="clock">00:00</div></div><div class="timer-controls"><button id="prev">← VORHERIGE ÜBUNG</button><button class="main" id="pause">PAUSE</button><button id="next">NÄCHSTE ÜBUNG →</button></div></section>`;
...
function jumpExercise(dir){
 ...
 if(mode==='full'||i===undefined){
   // determine current exercise from visible phase name
